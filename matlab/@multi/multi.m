classdef multi
    ...
    methods
        function obj=multi(cmd,varargin)
            if nargin==0
                obj.data=multi_mex('set_zeros',get_default_prec(),double(get_default_round_mode()),'r',0,0,0);
            elseif nargin==1 && isa(cmd,'multi')
                obj.data=multi_mex('copy',get_default_prec(),double(get_default_round_mode()),cmd.data);
            elseif nargin==1 && isa(cmd,'double')
                obj.data=multi_mex('set_d',get_default_prec(),double(get_default_round_mode()),cmd);
            elseif nargin==1 && (isnumeric(cmd) || islogical(cmd))
                obj.data=multi_mex('set_d',get_default_prec(),double(get_default_round_mode()),double(cmd));
            elseif nargin==1 && isa(cmd,'cell')
                obj.data=multi_mex('set_s',get_default_prec(),double(get_default_round_mode()),cmd);
            elseif nargin==1 && isa(cmd,'char')
                obj.data=multi_mex('set_s',get_default_prec(),double(get_default_round_mode()),strsplitcell(cmd));
            else
