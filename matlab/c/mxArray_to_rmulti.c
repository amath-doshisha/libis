...
void mxArray_to_rmulti(rmulti *y, const mxArray *x, int i, const char *field_names[])
{
  ...
  else{ MATLAB_ERROR("mxArrayToMulti: The arg should be Struct with the feild 'prec'."); }
  // sign
  value=mxGetField(x,i,field_names[1]);
  if(value!=NULL && mxIsInt32(value)){ y->_mpfr_sign=(*(int32_t*)mxGetData(value)); }
  else{ MATLAB_ERROR("mxArrayToMulti: The arg should be Struct with the feild 'sign'."); }
  // exp
  value=mxGetField(x,i,field_names[2]);
  if(value!=NULL && mxIsInt64(value)){ y->_mpfr_exp=(*(int64_t*)mxGetData(value)); }
  else{ MATLAB_ERROR("mxArrayToMulti: The arg should be Struct with the feild 'exp'."); }
  // digits
  ...
  else{ MATLAB_ERROR("mxArrayToMulti: The arg should be Struct with the feild 'digits'."); }
}
