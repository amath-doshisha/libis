...
void multi_mid(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if(!(IS_STRT(nrhs,prhs,N0))){ MATLAB_ERROR("multi_mid: The 1st-arg should be Struct."); }
  // allocate by clone
  ...
  else if(_T(x)=='R' && _T(y)=='r'){ irmat3_mid(_M(y),_N(y),_L(y),_R(y),_LD1(y),_LD2(y),_R0(x),_R1(x),_LD1(x),_LD2(x)); }
  else if(_T(x)=='c' && _T(y)=='c'){ cmat3_copy(_M(y),_N(y),_L(y),_C(y),_LD1(y),_LD2(y),_C(x),_LD1(x),_LD2(x)); }
  else if(_T(x)=='C' && _T(y)=='c'){ icmat3_mid(_M(y),_N(y),_L(y),_C(y),_LD1(y),_LD2(y),_C0(x),_C1(x),_LD1(x),_LD2(x)); }
  else{ MATLAB_ERROR("multi_mid: Unkown type"); }
  // done
