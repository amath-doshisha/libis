...
void multi_max(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if(nrhs==N0+1){
    if((_M(x)==1 && _L(x)==1) || (_N(x)==1 && _L(x)==1)){
      ...
      else if(_T(x)=='R'){ y=multi_allocate('R',1,1,1); irmax_rvec(_R0(y)[0],_R1(y)[0],_M(x)*_N(x)*_L(x),_R0(x),_R1(x)); }
      else if(_T(x)=='C'){ y=multi_allocate('C',1,1,1); icmax_cvec(_C0(y)[0],_C1(y)[0],_M(x)*_N(x)*_L(x),_C0(x),_C1(x)); }
    }else if(_L(x)==1){
