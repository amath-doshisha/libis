...
void multi_set_ones(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  // input-2
  if(!(IS_CHAR(nrhs,prhs,N0))){ MATLAB_ERROR("multi_set_ones: The 1st-arg should be Char."); }
  ...
  if(nrhs-N0==2 && IS_NUMR(nrhs,prhs,N0+1)){
    ...
    dim=ivec_allocate(ndim);    
    for(i=0; i<ndim; i++){
      if(!IS_NUMR(nrhs,prhs,N0+1+i)){ MATLAB_ERROR("multi_set_ones: The 2nd, 3rd, ... args should be scalar."); }
      dim[i]=GET_DOUBLE(prhs[N0+1+i])[0];
    }
  }else{ MATLAB_ERROR("multi_set_oens: The 2nd-arg should be scalar Double or row double vector."); }  
  // set_ones(x,type,size)
