...
void multi_set_zeros(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  }else{ MATLAB_ERROR("multi_set_zeros: The arg3 should be scalar Double or row double vector."); }  
  ...
  else if(_T(A)=='R'){ rmat3_set_zeros(_M(A),_N(A),_L(A),_R0(A),_LD1(A),_LD2(A)); rmat3_set_zeros(_M(A),_N(A),_L(A),_R1(A),_LD1(A),_LD2(A)); }
  else if(_T(A)=='C'){ cmat3_set_zeros(_M(A),_N(A),_L(A),_C0(A),_LD1(A),_LD2(A)); cmat3_set_zeros(_M(A),_N(A),_L(A),_C1(A),_LD1(A),_LD2(A)); }
