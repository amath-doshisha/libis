...
void multi_subsref(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if(s->ndim==1 && s->dims[0]==0 && s->index[0]==NULL){ // A(:)
    ...
    y=multi_allocate(_T(x),s->dims[0],s->dims[1],1);
         if(_T(x)=='r'){ rmat3_clone_rmat_index(_R(y),_LD1(y),_R(x),_LD1(x),_LD2(x),s->dims[0],s->index[0],s->dims[1],s->index[1]); }
    else if(_T(x)=='c'){ cmat3_clone_cmat_index(_C(y),_LD1(y),_C(x),_LD1(x),_LD2(x),s->dims[0],s->index[0],s->dims[1],s->index[1]); }
    else{ MATLAB_ERROR("multi_subsref: Unkown type"); }	           
  }else if(s->ndim==3){ // A(*,*,*)
    if(s->dims[0]==0 && s->index[0]==NULL){ s->dims[0]=_M(x); s->index[0]=ivec_allocate(s->dims[0]); ivec_grid(s->dims[0],s->index[0]); }
    ...
    y=multi_allocate(_T(x),s->dims[0],s->dims[1],s->dims[2]);
         if(_T(x)=='r'){ rmat3_clone_index(_M(y),_N(y),_L(y),_R(y),_LD1(y),_LD2(y),_R(x),_LD1(x),_LD2(x),s->index[0],s->index[1],s->index[2]); }
    else if(_T(x)=='c'){ cmat3_clone_index(_M(y),_N(y),_L(y),_C(y),_LD1(y),_LD2(y),_C(x),_LD1(x),_LD2(x),s->index[0],s->index[1],s->index[2]); }
