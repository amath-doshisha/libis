...
int main(int argc, char *argv[])
{
  ...
  char *out_fname_log=NULL,*out_fname_val=NULL,*out_fname_vec=NULL,*out_fname_val_e=NULL,*out_fname_vec_e=NULL,*out_dir=NULL,buf[10000];
  char format[]="f",method[METHOD_NAME_LENGTH+1];
  int n=0,k=0,debug=2,i,mX=0,nX=0,mA=0,nA=0,nLambda=0,prec=DEFAULT_PREC,autoname=1,digits=6,kmode=1,kprec=-1,kret=1;
  cmulti **cA=NULL,**cX=NULL,**cL=NULL,**cXe=NULL,**cLe=NULL,**cLambda=NULL;
  rmulti **rA=NULL,**rX=NULL,**rL=NULL,**rXe=NULL,**rLe=NULL,*rXe_max=NULL,*rXe_max_log2=NULL,*rLe_max=NULL,*rLe_max_log2=NULL,*Lmax=NULL,**rLambda=NULL;
 ...
 // output
  ...
  if(autoname){
    ...
    sprintf(buf,"%s_%cvec{%dbits|%s|eigval}.%s",path->str[1],mode,prec,method,BIN_SUFFIX); out_fname_val=char_cat(out_fname_val,buf);
    sprintf(buf,"%s_%cmat{%dbits|%s|eigvec}.%s",path->str[1],mode,prec,method,BIN_SUFFIX); out_fname_vec=char_cat(out_fname_vec,buf);
    if(kmode){
      sprintf(buf,"%s_%cvec{%dbits|%s|eigval}_%cvec{krawczyk|%dbits}.%s",path->str[1],mode,prec,method,mode,kprec,BIN_SUFFIX); out_fname_val_e=char_cat(out_fname_val_e,buf);
      sprintf(buf,"%s_%cmat{%dbits|%s|eigvec}_%cmat{krawczyk|%dbits}.%s",path->str[1],mode,prec,method,mode,kprec,BIN_SUFFIX); out_fname_vec_e=char_cat(out_fname_vec_e,buf);
    }
  ...
  if(debug>=1){
    printf("Output_File_Log:  %s%s\n",out_dir,(strlen(out_fname_log)>0?out_fname_log:"NULL"));fprintf(fid,"Output_File_Log:  %s%s\n",out_dir,(strlen(out_fname_log)>0?out_fname_log:"NULL"));
    printf("Output_File_Eigenvalues:  %s%s\n",out_dir,(strlen(out_fname_val)>0?out_fname_val:"NULL"));fprintf(fid,"Output_File_Eigenvalues:  %s%s\n",out_dir,(strlen(out_fname_val)>0?out_fname_val:"NULL"));
    printf("Output_File_Eigenvectors: %s%s\n",out_dir,(strlen(out_fname_vec)>0?out_fname_vec:"NULL"));fprintf(fid,"Output_File_Eigenvectors: %s%s\n",out_dir,(strlen(out_fname_vec)>0?out_fname_vec:"NULL"));
    if(kmode){
      printf("Output_File_Error_Bounds_Eigenvalues:  %s%s\n",out_dir,(strlen(out_fname_val_e)>0?out_fname_val_e:"NULL"));fprintf(fid,"Output_File_Error_Bounds_Eigenvalues:  %s%s\n",out_dir,(strlen(out_fname_val_e)>0?out_fname_val_e:"NULL")); 
      printf("Output_File_Error_Bounds_Eigenvectors: %s%s\n",out_dir,(strlen(out_fname_vec_e)>0?out_fname_vec_e:"NULL"));fprintf(fid,"Output_File_Error_Bounds_Eigenvectors: %s%s\n",out_dir,(strlen(out_fname_vec_e)>0?out_fname_vec_e:"NULL")); 
    }
  }
  ...
  if(mode_init=='R'){
    if(debug>=1){
      printf("Initial_Vectors: random\n");fprintf(fid,"Initial_Vectors: random\n");
    }
    mX=n; nX=n;
    ...
    if(debug>=1){
      printf("Initial_Vectors: loaded from %s\n",(strlen(init_fname)>0?init_fname:"NULL"));fprintf(fid,"Initial_Vectors: loaded from %s\n",(strlen(init_fname)>0?init_fname:"NULL"));
    }
    ...
    if(debug>=1){
      printf("Initial_Vectors: computed by II with %s\n",(strlen(init_fname)>0?init_fname:"NULL"));fprintf(fid,"Initial_Vectors: computed by II with %s\n",(strlen(init_fname)>0?init_fname:"NULL"));
    }
    //Load lambda
    cLambda=cvec_bin_load(&nLambda,"%s",init_fname);
    if(cLambda==NULL){ printf("Error! The input file '%s' can NOT be opened, or is NOT supported type.\n\n",init_fname); usage(); }
    if(rA!=NULL){
      rLambda=rvec_allocate_prec(nLambda,prec);
      cvec_real_clone(nLambda,rLambda,cLambda);
      cLambda=cvec_free(nLambda,cLambda);    
      cL=cvec_allocate_prec(n,prec); cvec_set_nan(n,cL);
      ...
      clock_start=clock();
      reig_ii(n,rX,mX,rA,mA,rLambda,debug-1);
      clock_end=clock();
      time_ii=(double)(clock_end-clock_start)/CLOCKS_PER_SEC;
      time_total+=time_ii;
      if(debug>=1){
	printf("Computing_Time_II: %.15f sec\n",time_ii);fprintf(fid,"Computing_Time_II: %.15f sec\n",time_ii);
      }
      rvec_clone(nLambda,rL,rLambda);
    }else if(cA!=NULL){
      cL=cvec_allocate_prec(n,prec); cvec_set_nan(n,cL);
      // computing eigenvectors by inverse iteration method
      mX=n; nX=n; cX=cmat_allocate_prec(mX,nX,prec);
      clock_start=clock();
      ceig_ii(n,cX,mX,cA,mA,cLambda,debug-1);
      clock_end=clock();
      time_ii=(double)(clock_end-clock_start)/CLOCKS_PER_SEC;
      time_total+=time_ii;
      if(debug>=1){
	printf("Computing_Time_II: %.15f sec\n",time_ii);fprintf(fid,"Computing_Time_II: %.15f sec\n",time_ii);
      ...
      cvec_clone(nLambda,cL,cLambda);
    }
  }else if(mode_init=='Q'){
    if(debug>=1){
      printf("Initial_Vectors: computed by QR\n");fprintf(fid,"Initial_Vectors: computed by QR\n");
    }
    if(rA!=NULL){
      ...
      time_total+=time_ii;
      if(debug>=1){
      printf("Computing_Time_QR: %.15f sec\n",time_qr);fprintf(fid,"Computing_Time_QR: %.15f sec\n",time_qr);
      printf("Computing_Time_II: %.15f sec\n",time_ii);fprintf(fid,"Computing_Time_II: %.15f sec\n",time_ii);
      }
    }else if(cA!=NULL){
      // computing eigenvalues by QR-method
  ...
  // output results
  if(debug>=1){
    printf("Num_Obtained_Vectos: %d\n",k);fprintf(fid,"Num_Obtained_Vectos: %d\n",k);
    printf("Num_Not_Obtained_Vectos: %d\n",n-k);fprintf(fid,"Num_Not_Obtained_Vectos: %d\n",n-k);
    printf("Computing_Time_HPEIG: %.15f sec\n",time_hpeig);fprintf(fid,"Computing_Time_HPEIG: %.15f sec\n",time_hpeig);
    printf("Computing_Time_Total: %.15f sec\n",time_total);fprintf(fid,"Computing_Time_Total: %.15f sec\n",time_total);
  }
  if(debug>=3){
    printf("Eigenvectors:\n");
  ...
  if(debug>=1){
    ...
    mpfr_fprintf(fid,"Eigenvalues_Max: %.7Re\n",Lmax);
  }
  if(debug>=2){
    printf("Eigenvalues:\n");
  ...
  // error bounds
  if(debug>=1){
    printf("Error_Bounds_Mode: %s\n",kmode?"Yes":"No");fprintf(fid,"Error_Bounds_Mode: %s\n",kmode?"Yes":"No");
  }
  if(kmode){
    if(debug>=1){
      printf("Error_Bounds_Prec: %d\n",kprec);fprintf(fid,"Error_Bounds_Prec: %d\n",kprec);
    }
	...
	time_k=(double)(clock_end-clock_start)/CLOCKS_PER_SEC;
      ...
      // computing
      clock_start=clock();
      kret=ireig_krawczyk(n,k,rXe,mX,rLe,rA,mA,rX,mX,rLambda,debug-1);
      clock_end=clock();
      time_k=(double)(clock_end-clock_start)/CLOCKS_PER_SEC;
      }else if(cA!=NULL && cX!=NULL && cL!=NULL){
	cXe=cmat_allocate_prec(mX,nX,kprec);
	...
	// computing
	clock_start=clock();
	kret=iceig_krawczyk(n,k,cXe,mX,cLe,cA,mA,cX,mX,cLambda,debug-1);
	clock_end=clock();
	time_k=(double)(clock_end-clock_start)/CLOCKS_PER_SEC;
      }else{ ERROR_AT; exit(0); }
    }
    ...
    // output
    if(debug>=1){
      printf("Error_Bounds_Result: %s\n",kret?"No":"Yes");fprintf(fid,"Error_Bounds_Result: %s\n",kret?"No":"Yes");
      printf("Computing_Time_Error_Bounds: %.15f sec\n",time_k);fprintf(fid,"Computing_Time_Error_Bounds: %.15f sec\n",time_k);
    }
    if(kret){
      // failed
	...
	mpfr_fprintf(fid,"Error_Bounds_Eigenvectors_Max: %.1Re 2^%d\n",rXe_max,(int)ceil(rget_d(rXe_max_log2)));
	mpfr_printf("Error_Bounds_Eigenvalues_Max_Per_Max: %.1Re 2^%d\n",rLe_max,(int)ceil(rget_d(rLe_max_log2)));
	mpfr_fprintf(fid,"Error_Bounds_Eigenvalues_Max_Per_Max: %.1Re 2^%d\n",rLe_max,(int)ceil(rget_d(rLe_max_log2)));
      }
      if(debug>=3){
  ...
  cL=cvec_free(n,cL);
  rL=rvec_free(n,rL);
  rLambda=rvec_free(nLambda,rLambda);
  cLambda=cvec_free(nLambda,cLambda);
  cLe=cvec_free(n,cLe);
  rLe=rvec_free(n,rLe);
