...
/**
 @brief 足し算 [z0,z1]=[x0,x1]+[y0,y1]
*/
void icvec_add_c(int n, cmulti **z0, cmulti **z1, cmulti **x0, cmulti **x1, cmulti *y0, cmulti *y1)
{
  int i;
...
void icvec_func(cmulti *y0, cmulti *y1, func_t *f, int n, cmulti **x0, cmulti **x1)
{
  ...
  else if(func_is_zero(f))   { icset_dd(z0,z1,0,0); }
  else if(func_is_one(f))    { icset_dd(z0,z1,1,0); }
