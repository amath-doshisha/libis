...
{
  func_t *f=NULL;
  if     (func_is(f1,"rvec") && func_is(f2,"rvec") && FRS(f1)==FRS(f2)){ f=func_rvec(FRS(f1)); rvec_add_rvec_rvec  (FRS(f),FRV(f),FRV(f1),FRV(f2)); }
  else if(func_is(f1,"rvec") && func_is(f2,"cvec") && FRS(f1)==FCS(f2)){ f=func_cvec(FRS(f1)); cvec_add_cvec_rvec(FCS(f),FCV(f),FCV(f2),FRV(f1)); }
  else if(func_is(f1,"cvec") && func_is(f2,"rvec") && FCS(f1)==FRS(f2)){ f=func_cvec(FCS(f1)); cvec_add_cvec_rvec(FCS(f),FCV(f),FCV(f1),FRV(f2)); }
  else if(func_is(f1,"cvec") && func_is(f2,"cvec") && FCS(f1)==FCS(f2)){ f=func_cvec(FCS(f1)); cvec_add_cvec_cvec  (FCS(f),FCV(f),FCV(f1),FCV(f2)); }
...
{
  func_t *f=NULL;
  if     (func_is(f1,"rvec") && func_is(f2,"rvec") && FRS(f1)==FRS(f2)){ f=func_rvec(FRS(f1)); rvec_sub_rvec_rvec   (FRS(f),FRV(f),FRV(f1),FRV(f2)); }
  else if(func_is(f1,"rvec") && func_is(f2,"cvec") && FRS(f1)==FCS(f2)){ f=func_cvec(FRS(f1)); cvec_sub_rvec_cvec(FCS(f),FCV(f),FRV(f1),FCV(f2)); }
  else if(func_is(f1,"cvec") && func_is(f2,"rvec") && FCS(f1)==FRS(f2)){ f=func_cvec(FCS(f1)); cvec_sub_cvec_rvec(FCS(f),FCV(f),FCV(f1),FRV(f2)); }
  else if(func_is(f1,"cvec") && func_is(f2,"cvec") && FCS(f1)==FCS(f2)){ f=func_cvec(FCS(f1)); cvec_sub_cvec_cvec   (FCS(f),FCV(f),FCV(f1),FCV(f2)); }
