...
void reig_hqr_main(int n, cmulti **lambda, rmulti **A, int LDA, int debug)
{
  ...
  while(nn>=1){
	...
	// 実固有値に収束している場合
	// lambda(n-1)=x+t
	radd_r(C_R(lambda[nn-1]),x,t); rset_zero(C_I(lambda[nn-1]));
	...
	rmul_r(w,MAT(A,nn-1,nn-2,LDA),MAT(A,nn-2,nn-1,LDA));
	//
	if(l==(nn-1)){
	  // p=(y-x)/2
	  rsub_r(p,y,x); rmul_d(p,p,0.5);
	  // q=(y-x)^2/4+w
	  rmul_r(q,p,p); radd_r(q,q,w);
	  ...
	  if(ris_positive(q) || ris_zero(q)){
	    ...
	    if(ris_zero(z)){
	      // lambda(n-1)=x+z
	      radd_r(C_R(lambda[nn-1]),x,z); rset_zero(C_I(lambda[nn-1]));
	    }else{
	      // lambda(n-1)=x-w/z
	      rdiv_r(C_R(lambda[nn-1]),w,z); rsub_r(C_R(lambda[nn-1]),x,C_R(lambda[nn-1])); rset_zero(C_I(lambda[nn-1]));
	    }
	    // lambda(n-2)=x+z
	    radd_r(C_R(lambda[nn-2]),x,z); rset_zero(C_I(lambda[nn-2]));
	  }else{
	    // 複素固有値に収束している場合
	    // lambda(n-1)=(x+p)+I*z
	    radd_r(C_R(lambda[nn-1]),x,p); rcopy(C_I(lambda[nn-1]),z);
	    // lambda(n-2)=(x+p)-I*z
	    radd_r(C_R(lambda[nn-2]),x,p); rneg(C_I(lambda[nn-2]),z);
	  }
	  if(debug>=1){
	  ...
	  if((its%10)==0){
	    ...
	    radd_r(t,t,x);
	    // 原点シフト
	    for(i=0; i<nn; i++){ rsub_r(MAT(A,i,i,LDA),MAT(A,i,i,LDA),x); }
	  ...
	  for(k=m; k<=nn-1; k++){
		...
		rneg(MAT(A,k-1,k-2,LDA),MAT(A,k-1,k-2,LDA)); 
	      ...
	      rdiv_r(r,r,p);
	      for(j=k;j<=nn;j++){
		rmul_r(p,q,MAT(A,k,j-1,LDA));
		radd_r(p,p,MAT(A,k-1,j-1,LDA));
		if(k!=(nn-1)){
		...
		rsub_r(MAT(A,i-1,k-1,LDA),MAT(A,i-1,k-1,LDA),p);
	      }
	    }
	  }
	}
...
{
  rmul_r(p,r,s);  // p=r*s
  rsub_r(p,p,w);  // p=r*s-w
  rdiv_r(p,p,a1); // p=(r*s-w)/a1
  radd_r(p,p,a2); // p=(r*s-w)/a1+a2
}
...
{
  rsub_r(q,a,z); // q=a-z
  rsub_r(q,q,r); // q=a-(z+r)
  rsub_r(q,q,s); // q=a-(z+r+s)
}
...
void reig_hqr_calculate_formula_11_6_25(rmulti *s, rmulti *p, rmulti *q, rmulti *r)
{
  ...
  rmul_r(X,q,q); // X=q^2
  rmul_r(Y,r,r); // Y=r^2
  radd_r(s,s,X); // s=p^2+q^2
  radd_r(s,s,Y); // s=p^2+q^2+r-2
...
void reig_hqr_calculate_formula_11_6_26_u(rmulti *u, rmulti *a, rmulti *q, rmulti *r)
{
  ...
  rabs(u,a);          // u=abs(a)
  rmul_r(u,u,X);        // u=abs(a)*(abs(q)+abs(r))
  // done
  RF(X);
}
...
void reig_hqr_calculate_formula_11_6_26_v(rmulti *v, rmulti *a1, rmulti *a2, rmulti *p, rmulti *z)
{
  ...
  rabs(v,p);              // v=abs(p)
  rmul_r(v,v,X);            // v=abs(p)*(abs(a1)+abs(z)+abs(a2))
  // done
  RF(X);
}
