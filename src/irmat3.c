...
void irmat3_set_d(int m, int n, int l, rmulti **B0, rmulti **B1, int LDB1, int LDB2, double *A, int LDA1, int LDA2)
{
  ...
  for(k=0; k<l; k++){
    for(j=0; j<n; j++){
      for(i=0; i<m; i++){
	irset_d(MAT3(B0,i,j,k,LDB1,LDB2),MAT3(B1,i,j,k,LDB1,LDB2),MAT3(A,i,j,k,LDA1,LDA2));
      }
    }
  }
}
...
void irmat3_set_dd(int m, int n, int l, rmulti **C0, rmulti **C1, int LDC1, int LDC2, double *A, int LDA1, int LDA2, double *B, int LDB1, int LDB2)
{
  ...
  for(k=0; k<l; k++){
    for(j=0; j<n; j++){
      for(i=0; i<m; i++){
	irset_dd(MAT3(C0,i,j,k,LDC1,LDC2),MAT3(C1,i,j,k,LDC1,LDC2),MAT3(A,i,j,k,LDA1,LDA2),MAT3(B,i,j,k,LDB1,LDB2));
      }
    }
  }
}
