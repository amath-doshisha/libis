...
{
  int i,mat=MAT_RAND,n=10,LDA,prec=53,prec2=512,e,info;
  ...
  printf("================\n");
  printf("[デフォルト精度で計算]\n");
  rmat_copy(n,n,A,LDA,A0,LDA);  rvec_copy(n,x,b);  e=rsolve(n,1,x,n,A,LDA,&info); rvec_copy(n,y,x);
  if(info==0){
    printf("(R=%d) ",e);  rvec_print_prec(n,x,"x=","e",20);
    printf("[無誤差で残差を計算]\n");
    set_auto_prec_enabled();
    e=rsolve_residual(n,1,r,n,A0,LDA,x,n,b,n);
    set_auto_prec_disabled();
    printf("(R=%d) ",e);
  ...
  printf("================\n");
  printf("[精度%dbitsで計算]\n",prec2);
  set_default_prec(prec2); rmat_round(n,n,A,LDA,prec2); rvec_round(n,x,prec2);
  rmat_copy(n,n,A,LDA,A0,LDA);  rvec_copy(n,x,b);  e=rsolve(n,1,x,n,A,LDA,&info);
  set_default_prec(prec);
  if(info==0){
    printf("(R=%d) ",e);  rvec_print_prec(n,x,"x=","e",20);
    printf("[無誤差で残差を計算]\n");
    ...
    rvec_round(n,r,prec);
    e=rvec_sub(n,r,x,y);
    e+=rvec_div_2exp(n,r,r,rvec_get_prec_max(n,x));
    printf("(R=%d) ",e);
